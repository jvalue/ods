...
import NotificationConfig, { NotificationParameters, NotificationType } from '@/notification/notificationConfig';
...
export async function getById(id: number): Promise<NotificationConfig> {
  ...
  const notificationApiModel = JSON.parse(response.data) as NotificationApiReadModel;
