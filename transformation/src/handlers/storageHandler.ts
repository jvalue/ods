...
export class StorageHandler implements PipelineRepository{
    ...
    */
    public async getPipelineConfigs(queryParams: object): Promise<PipelineConfig[] | null> {
    ...
    public async savePipelineConfig(pipelineConfig: PipelineConfig): Promise<PipelineConfig>{
        ...
        // persist the Config
        const saveResult = await this.configRepository.save(pipelineConfig);

        // Remove (autogenerated) id Entries from metadata and pipeline entry
        delete saveResult.metadata.id
        delete saveResult.transformation.id
    ...
    public async updatePipelineConfig(id: number, pipelineConfig: PipelineConfig): Promise<PipelineConfig> {
        ...
        try {
            updatedConfig = await this.configRepository.save(pipelineConfig)           
        } catch (error) {        
        ...
        // Remove id Entries from metadata and pipeline entry (autogenerated)
        delete updatedConfig.metadata.id
        delete updatedConfig.transformation.id
