...
export class DatasourceExecutionConsumer {
  constructor(private readonly amqpChannel: AmqpChannel, private readonly pipelineManager: PipelineConfigManager) {}
  ...
  consumeEvent = async (msg: ConsumeMessage | null): Promise<void> => {
    ...
    if (msg.fields.routingKey === AMQP_DATASOURCE_EXECUTION_SUCCESS_TOPIC) {
      const triggerRequest = JSON.parse(msg.content.toString()) as PipelineConfigTriggerRequest;
