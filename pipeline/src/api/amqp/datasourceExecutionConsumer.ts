...
export async function createDatasourceExecutionConsumer(
  amqpConnection: AmqpConnection,
  pipelineManager: PipelineConfigManager,
): Promise<DatasourceExecutionConsumer> {
  const channel = await amqpConnection.createChannel();
  const datasourceExecutionConsumer = new DatasourceExecutionConsumer(
    channel,
    pipelineManager,
  );
...
export class DatasourceExecutionConsumer {
  ...
  async init(): Promise<void> {
    ...
    await this.amqpChannel.assertQueue(AMQP_DATASOURCE_EXECUTION_QUEUE, {
      exclusive: false,
    });
  ...
  consumeEvent = async (msg: ConsumeMessage | null): Promise<void> => {
    ...
    if (msg.fields.routingKey === AMQP_DATASOURCE_EXECUTION_SUCCESS_TOPIC) {
      const triggerRequest = JSON.parse(
        msg.content.toString(),
      ) as PipelineConfigTriggerRequest;
      await this.pipelineManager.triggerConfig(
        triggerRequest.datasourceId,
        JSON.parse(triggerRequest.data),
      );
    } else {
      console.debug(
