...
export class PipelineTransformedDataManager {
  ...
  async insert(
    ...
    return await this.pgClient.transaction(async (client) => {
      const savedConfig =
        await pipelineTransformedDataRepository.insertTransformedData(
          client,
          transformedData,
        );
      return savedConfig;
  ...
  async get(id: number): Promise<PipelineTransformedData | undefined> {
    return await this.pgClient.transaction(
      async (client) => await pipelineTransformedDataRepository.get(client, id),
    );
  }

  async getAll(): Promise<PipelineTransformedData[]> {
    return await this.pgClient.transaction(
      async (client) => await pipelineTransformedDataRepository.getAll(client),
    );
  }
  ...
  async getByPipelineId(
    ...
    return await this.pgClient.transaction(
      async (client) =>
        await pipelineTransformedDataRepository.getByPipelineId(
          client,
          pipelineId,
        ),
    );
  ...
  async update(
    ...
    return await this.pgClient.transaction(async (client) => {
      await pipelineTransformedDataRepository.update(
        client,
        id,
        transformedData,
      );
