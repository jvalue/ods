...
export class PipelineTransformedDataManager {
  constructor(private readonly pgClient: PostgresClient) {}
  ...
  async get(id: number): Promise<PipelineTransformedData | undefined> {
    return await this.pgClient.transaction(async (client) => await pipelineTransformedDataRepository.get(client, id));
  }

  async getAll(): Promise<PipelineTransformedData[]> {
    return await this.pgClient.transaction(async (client) => await pipelineTransformedDataRepository.getAll(client));
  }

  async getLatest(id: number): Promise<PipelineTransformedData | undefined> {
    return await this.pgClient.transaction(
      async (client) => await pipelineTransformedDataRepository.getLatest(client, id),
    );
  }

  async getByPipelineId(pipelineId: number): Promise<PipelineTransformedData[]> {
    return await this.pgClient.transaction(
      async (client) => await pipelineTransformedDataRepository.getByPipelineId(client, pipelineId),
    );
  }
