...
import { InsertStorageContent, StorageContent, StorageContentRepository } from './storageContentRepository';
...
async function existsTable(client: ClientBase, tableIdentifier: string): Promise<boolean> {
  const resultSet: QueryResult<ExistsTableResultRow> = await client.query(EXISTS_TABLE_STATEMENT(tableIdentifier));
