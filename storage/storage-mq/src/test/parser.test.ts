...
describe('schema generation', () => {
  test('return valid jsonschema for Pegel based on the ontology schema', async () => {
    const schemaParser = new SchemaParser()
    const response =
      await schemaParser.parse(testData.JSONSchemaPegelComplete, testData.MultiCompletePegel, 'TESTSCHEMA', 'TESTTABLE', 0)
...
describe('schema generation', () => {
  test('return valid jsonschema for Pegel based on the ontology schema', async () => {
    const schemaParser = new SchemaParser()
    const response =
      await schemaParser.parse(testData.JSONSchemaOrdngungsamtComplete, testData.MultiCompleteOrdnungsamt, 'TESTSCHEMA', 'TESTTABLE', 0)
